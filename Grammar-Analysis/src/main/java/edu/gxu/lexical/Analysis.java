...
//                    }
                ...
                if (Util.isKeyword(matches.toString())) {
                    addToken(matches, TokenTypeEnum.Keyword.getType(), Util.getKeywordCode(matches));
                } else {
...
//                while ((!isEndChar(ch)) && (Util.isDigit(ch) || Util.isDigitChar(ch))) {
                ...
                addToken(matches, TokenTypeEnum.NumberConst.getType(), constCode);
                continue;
            }
            // '开头，字符
            if (ch == '\'') {
...
//                    }
            ...
            if (ch == '/') {
                matches.append(ch);
                if(!canGetNextChar()){
                    addToken(matches, TokenTypeEnum.Operator.getType(), Util.getOperatorCode(matches.toString()));
                }
                else{
                    ch = getNextChar();
                    int state = 3;
            ...
            if (Util.isDelimiter(String.valueOf(ch))) {
                matches.append(ch);
                addToken(matches, TokenTypeEnum.Delimiter.getType(), Util.getDelimiterCode(matches.toString()));
                continue;
            }
            // 剩下的是非法情况
            else {
